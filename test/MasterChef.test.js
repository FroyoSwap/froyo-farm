...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await FroyoReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.froyo.address, '100', '1000', { from: owner });

        await this.froyo.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setFroyoReferral(this.referral.address, { from: owner });
