...
// FroyoToken with Governance.
contract FroyoToken is BEP20 {
    ...
    // The swap router, modifiable. Will be changed to FroyoSwap's router when our own AMM release
    IUniswapV2Router02 public froyoSwapRouter;
    // The trading pair
    address public froyoSwapPair;
    // In swap and liquify
    ...
    event FroyoSwapRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    /**
     * @notice Constructs the FroyoToken contract.
     */
    constructor() public BEP20("FroyoSwap Token", "PANTHER") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && _inSwapAndLiquify == false
            && address(froyoSwapRouter) != address(0)
            && froyoSwapPair != address(0)
            && sender != froyoSwapPair
            && sender != owner()
        ) {
            swapAndLiquify();
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the froyoSwap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = froyoSwapRouter.WETH();

        _approve(address(this), address(froyoSwapRouter), tokenAmount);

        // make the swap
        froyoSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        froyoSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from froyoSwapRouter when swapping
     ...
     * Can only be called by the current operator.
     */
    function updateFroyoSwapRouter(address _router) public onlyOperator {
        froyoSwapRouter = IUniswapV2Router02(_router);
        froyoSwapPair = IUniswapV2Factory(froyoSwapRouter.factory()).getPair(address(this), froyoSwapRouter.WETH());
        require(froyoSwapPair != address(0), "PANTHER::updateFroyoSwapRouter: Invalid pair address.");
        emit FroyoSwapRouterUpdated(msg.sender, address(froyoSwapRouter), froyoSwapPair);
    }
